...
TERRAIN::TERRAIN()
{
	m_pDevice = NULL;
	m_pMapTiles = NULL;
	m_pVisitedTiles = m_pVisibleTiles = NULL;
	m_pLandScape = m_pLightMap = m_pFogOfWarTexture = NULL;
}
...
void TERRAIN::Init(IDirect3DDevice9* Dev, INTPOINT _size)
{
	...
	if(FAILED(D3DXCreateTextureFromFile(Dev, "textures/mountain.jpg", &mount)))	debug.Print("Could not load stone.dds");
	if(FAILED(D3DXCreateTextureFromFile(Dev, "textures/snow.jpg", &snow)))		debug.Print("Could not load snow.dds");
	...
	//Fog-of-War texture
	if(FAILED(m_pDevice->CreateTexture(256, 256, 1, D3DUSAGE_RENDERTARGET, D3DFMT_A8R8G8B8, D3DPOOL_DEFAULT, &m_pFogOfWarTexture, NULL)))debug.Print("Failed to create texture: m_pFogOfWarTexture");

	//Terrain Texture
	if(FAILED(m_pDevice->CreateTexture(256, 256, 1, D3DUSAGE_RENDERTARGET, D3DFMT_A8R8G8B8, D3DPOOL_DEFAULT, &m_pLandScape, NULL)))debug.Print("Failed to create texture: m_pLandScape");
	...
	Progress("Creating Terrain", 0.0f);
	...
	//Create visited & m_visible tiles
	if(m_pVisitedTiles)delete [] m_pVisitedTiles;
	if(m_pVisibleTiles)delete [] m_pVisibleTiles;
	m_pVisitedTiles = new bool[m_size.x * m_size.y];
	m_pVisibleTiles = new bool[m_size.x * m_size.y];
	memset(m_pVisitedTiles, false, sizeof(bool)* m_size.x * m_size.y);
	memset(m_pVisibleTiles, false, sizeof(bool)* m_size.x * m_size.y);

	Progress("Creating Terrain", 0.4f);

	GenerateRandomTerrain(15);
}
...
void TERRAIN::Release()
{
	...
	{
		debug.Print("Error in TERRAIN::Release()");
	}
}
...
void TERRAIN::GenerateRandomTerrain(int numPatches)
{
	try
	{
		...
		hm3.LoadFromFile(m_pDevice, "heightmaps/four_players.jpg");
		...
		*m_pHeightMap *= hm3;
		...
		//Add objects
		HEIGHTMAP hm4(m_size, 1.0f);
		hm4.CreateRandomHeightMap(rand()%1000, 5.5f, 0.9f, 7);

		for(int y=0;y<m_size.y;y++)
			for(int x=0;x<m_size.x;x++)
			{
				if(m_pHeightMap->GetHeight(x, y) == 0.0f && hm4.GetHeight(x, y) > 0.7f && rand()%12 == 0)
					AddObject(0, INTPOINT(x, y));	//Tree
				else if(m_pHeightMap->GetHeight(x, y) >= 1.0f && hm4.GetHeight(x, y) > 0.9f && rand()%30 == 0)
...
void TERRAIN::CalculateLightMap(bool allWhite)
{
	try
	{
		//Clear old alpha maps
		...
		D3DXCreateTexture(m_pDevice, 128, 128, 1, D3DUSAGE_DYNAMIC, D3DFMT_L8, D3DPOOL_DEFAULT, &m_pLightMap);

		//Lock the texture
		D3DLOCKED_RECT sRect;
		m_pLightMap->LockRect(0, &sRect, NULL, NULL);
		BYTE *bytes = (BYTE*)sRect.pBits;
		memset(bytes, 255, sRect.Pitch*sRect.Pitch);		//Clear texture to white
...
void TERRAIN::Render(CAMERA &camera)
{
	...
	m_pDevice->SetTexture(4, m_pFogOfWarTexture);		//Fog-of-War
	...
	for(int p=0;p<(int)m_patches.size();p++)
		if(m_patches[p]->m_visible && !camera.Cull(m_patches[p]->m_BBox))
			m_patches[p]->Render();
	...
	m_pDevice->SetTexture(1, m_pFogOfWarTexture);		//Lightmap
	
	m_objectsVS.Begin();
	m_objectsPS.Begin();

	for(int i=0;i<(int)m_objects.size();i++)
		if(m_objects[i].m_visible && !camera.Cull(m_objects[i].m_BBox))
		{
...
{
	m_pDevice->Clear(0, 0, D3DCLEAR_TARGET | D3DCLEAR_ZBUFFER, 0x00000000, 1.0f, 0L);
	m_pDevice->BeginScene();
	
	RECT rc = {200, 250, 600, 300};
	m_pProgressFont->DrawText(NULL, text.c_str(), -1, &rc, DT_CENTER | DT_VCENTER | DT_NOCLIP, 0xffffffff);
...
void TERRAIN::SetOrthogonalView()
{
	...
	D3DXVECTOR2 center = D3DXVECTOR2((m_size.x - 1) / 2.0f, -(m_size.y - 1) / 2.0f);
	...
	//Projection matrix
	D3DXMatrixOrthoLH(&proj, m_size.x - 1.0f, m_size.y - 1.0f, 0.1f, 2000.0f);
...
void TERRAIN::UpdateSightMatrices(std::vector<MAPOBJECT*> &mapObjects)
{
	memset(m_pVisibleTiles, false, sizeof(bool) * m_size.x * m_size.y);
	for(int i=0;i<(int)mapObjects.size();i++)
		if(mapObjects[i] != NULL)
		{
			int sr = (int)mapObjects[i]->m_sightRadius, a = 0;
			INTPOINT start = mapObjects[i]->m_mappos - INTPOINT(sr, sr);
			INTPOINT end = mapObjects[i]->m_mappos + INTPOINT(sr, sr);
			...
			for(int y=start.y;y<=end.y;y++)
				for(int x=start.x;x<=end.x;x++)
					if(x >= 0 && y >= 0 && x < m_size.x && y < m_size.y)
					{
						...
						m_pVisitedTiles[index] = m_pVisibleTiles[index] = true;
					}
		}

	//Calculate visibility variable of Patches
	for(int i=0;i<(int)m_patches.size();i++)
		if(m_patches[i] != NULL)
		{
			m_patches[i]->m_visible = false;
			RECT r = m_patches[i]->m_mapRect;

			for(int y=r.top;y<=r.bottom && !m_patches[i]->m_visible;y++)
				for(int x=r.left;x<=r.right && !m_patches[i]->m_visible;x++)
					if(m_pVisitedTiles[x + y * m_size.x])
						m_patches[i]->m_visible = true;
		}

	//Calculate visibility of terrain objects
	for(int i=0;i<(int)m_objects.size();i++)
		m_objects[i].m_visible = m_pVisitedTiles[m_objects[i].m_mappos.x + m_objects[i].m_mappos.y * m_size.x];
}
...
void TERRAIN::RenderLandscape()
{
	...
	//Render Terrain
	{
		...
		//Create completely white lightmap
		IDirect3DTexture9* white = NULL;
		m_pDevice->CreateTexture(32, 32, 1, D3DUSAGE_DYNAMIC, D3DFMT_L8, D3DPOOL_DEFAULT, &white, NULL);
		D3DLOCKED_RECT sRect;
		white->LockRect(0, &sRect, NULL, NULL);
		memset((BYTE*)sRect.pBits, 255, 32 * 32);
		white->UnlockRect(0);
		...
		m_pDevice->SetTexture(2, m_diffuseMaps[1]);		//Mountain
		m_pDevice->SetTexture(3, m_diffuseMaps[2]);		//Snow
		m_pDevice->SetTexture(4, white);				//Use White Texture
		m_pDevice->SetMaterial(&m_mtrl);
		...
		//Set vertex shader variables
		m_terrainVS.SetMatrix(m_vsMatW, world);
		m_terrainVS.SetMatrix(m_vsMatVP, view * proj);
		m_terrainVS.SetVector3(m_vsDirToSun, m_dirToSun);
		...
		//Render Objects
		m_objectsVS.SetMatrix(m_objMatW, world);
		m_objectsVS.SetMatrix(m_objMatVP, view * proj);
		m_objectsVS.SetVector3(m_objDirToSun, m_dirToSun);
		m_objectsVS.SetVector3(m_objMapSize, D3DXVECTOR3((float)m_size.x, (float)m_size.y, 0.0f));
	...
	//D3DXSaveTextureToFile("m_pLandScape.bmp", D3DXIFF_BMP, m_pLandScape, NULL);
}
			...
			if(tile != NULL && tile->m_walkable)
			{
				...
				for(int i=0;i<8;i++)
					if(Within(p[i]))
					{
						...
						//Connect tiles if the neighbor is m_walkable
						if(neighbor != NULL && neighbor->m_walkable)
							tile->neighbors[i] = neighbor;
					}
			}
					...
					if(tile != NULL && tile->m_walkable)
					{
						for(int i=0;i<8;i++)
							if(tile->neighbors[i] != NULL &&
								tile->neighbors[i]->m_walkable &&
								tile->neighbors[i]->m_set < tile->m_set)
							{
								changed = true;
								tile->m_set = tile->neighbors[i]->m_set;
							}
					}
				}
		}
...
std::vector<INTPOINT> TERRAIN::GetPath(INTPOINT start, INTPOINT goal, bool considerUnits, MAPOBJECT *unit)
{
	try
	{
		...
		{	
			...
			open.erase(open.begin() + bestPlace);	// Take the best node out of the Open list
...
INTPOINT TERRAIN::GetClosestFreeTile(INTPOINT to, INTPOINT from)
{
	MAPTILE *tile = GetTile(to);
	if(tile != NULL && tile->m_walkable && tile->m_pMapObject == NULL)
		return to;

	try
	{
		for(int i=0;i<3;i++)	//Search radius
		{
			std::vector<MAPTILE*> tiles;

			//Get tiles in current search radius
			for(int y=to.y - i;y<=to.y + i;y++)
			{
				tiles.push_back(GetTile(to.x - i, y));
				tiles.push_back(GetTile(to.x + i, y));
			}

			for(int x=to.x - i + 1;x<=to.x + i - 1;x++)
			{
				tiles.push_back(GetTile(x, to.y - i));
				tiles.push_back(GetTile(x, to.y + i));
			}
			...
			for(int t=0;t<(int)tiles.size();t++)
				if(tiles[t] != NULL)
					if(tiles[t]->m_walkable && tiles[t]->m_pMapObject == NULL)
					{
						float d = from.Distance(tiles[t]->m_mappos);
						if(d < dist)
						{
							dist = d;
							closest = tiles[t];
			...
			if(closest != NULL)
				return closest->m_mappos;
		}
	}
	catch(...)
	{
		debug.Print("Error in TERRAIN::GetClosestFreeTile()");
	}

	return to;
}

bool TERRAIN::PositionAccessible(MAPOBJECT *unit, INTPOINT position)
{
	if(unit == NULL || !Within(unit->m_mappos) || !Within(position))return false;

	//Set start and end location of the search
	INTPOINT start = unit->m_mappos;
	INTPOINT end = position;
	...
	//Set the open variable of search area's mapTiles to false
	int searchBorder = 3;
	for(int y=start.y - searchBorder;y<=end.y + searchBorder;y++)
		for(int x=start.x - searchBorder;x<=end.x + searchBorder;x++)
			if(Within(INTPOINT(x, y)))
				m_pMapTiles[y * m_size.x + x].open = false;

	//Set the open variable of the start location to true
	m_pMapTiles[position.y * m_size.x + position.x].open = true;
	searchBorder--;

	//Iterate any open nodes to its neighbors if they are unoccupied by units
	bool changed;
	...
	do
	{
		...
		for(int y=start.y - searchBorder;y<=end.y + searchBorder;y++)
			for(int x=start.x - searchBorder;x<=end.x + searchBorder;x++)
				if(Within(INTPOINT(x, y)) && !m_pMapTiles[y * m_size.x + x].open)
					for(int i=0;i<8;i++)
					{
						MAPTILE *t = m_pMapTiles[y * m_size.x + x].neighbors[i];
						if(t != NULL && t->open && (t->m_pMapObject == NULL || t->m_pMapObject == unit))
						{
							m_pMapTiles[y * m_size.x + x].open = true;
							changed = true;
							i = 8;
						}
					}

		//If the open variable of the end node is true then a path exists
		if(m_pMapTiles[unit->m_mappos.y * m_size.x + unit->m_mappos.x].open)
			return true;
	}
	while(changed);
